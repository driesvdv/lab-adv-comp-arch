...
int main(void)
{
    ...
    file << "Array Size,CPU Time (ms),Atomic Time (ms),Reduction Time (ms)\n";

    for (int power = 8; power <= 20; ++power)
    {
        int numElements = 1 << power;            // Number of elements in the array
        ...
        for (int chunkIndex = 0; chunkIndex < numChunks; ++chunkIndex)
        {
            ...
            int end = std::min(start + chunkSize - 1, numElements - 1); // End index of the current chunk

            // Calculate the size of the current chunk
            int currentChunkSize = end - start + 1;

            // Timing for reduction
            int sizeHelper = min(1024, numElements);
            dim3 blockDim_reduction(sizeHelper);                                                          // block dimension (assuming each block has 1024 threads)
        ...
        // Record execution times in CSV
        file << numElements << "," << cpu_time.count() << "," << atomic_time.count() << "," << reduction_time.count() << "\n";
