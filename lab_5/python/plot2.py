...
data1 = pd.read_csv("timings/synchronous_cuda_results_2.csv")
x1 = data1["Array size"]
total_exec_time1 = data1[" Total Execution time (µs)"]

# Read data from file 2
data2 = pd.read_csv("timings/asynchronous_cuda_results_2.csv")
x2 = data2["Array Size"]
...
plt.title("Total Execution Time vs. Array Size on complexicated GPU functions")
plt.xscale("log")
plt.yscale("log")
plt.grid(True)
plt.legend()
