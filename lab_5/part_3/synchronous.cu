    ...
    if (idx < numElements)
    {
        while (numElements > 1)
        {
            if (idx < numElements / 2)
            {
                ...
                x[idx] *= x[idx + numElements / 2] + temp;
            }
            __syncthreads();
            numElements /= 2;
        }
        ...
        while (numElements > 1)
        {
            if (idx < numElements / 2)
            {
                if (input[idx] > input[idx + numElements / 2])
                {
                    ...
                    input[idx] *= input[idx + numElements / 2] + temp;
                }
            }
            __syncthreads();
            numElements /= 2;
...
int main(void)
{
    ...
    std::ofstream csv_file("./../timings/synchronous_cuda_results_2.csv");
    csv_file << "Array size, GPU execution time (µs), Data Prep time (µs), Data malloc time (µs), Total Execution time (µs)";
    ...
    for (int arr_size = 10; arr_size <= ARR_SIZE_MAX; arr_size *= 10)
    {
        ...
        float summation_time = 0, multiplication_time = 0, minimum_time = 0, maximum_time = 0, data_prep_time = 0, data_malloc_time = 0;
        for (int iter = 0; iter < NUM_ITERATIONS; iter++)
        {
            ...
            auto start_data_malloc = std::chrono::high_resolution_clock::now();
            ...
            auto end_data_malloc = std::chrono::high_resolution_clock::now();
            data_malloc_time += std::chrono::duration_cast<std::chrono::microseconds>(end_data_malloc - start_data_malloc).count();
        ...
        data_malloc_time /= NUM_ITERATIONS;

        auto execution_time = summation_time + multiplication_time + minimum_time + maximum_time;
        auto total_time = execution_time + data_prep_time + data_malloc_time;

        // Write results to CSV
        csv_file << "\n"
                 << arr_size << ", " << execution_time << ", " << data_prep_time << ", " << data_malloc_time << ", " << total_time;
    }
